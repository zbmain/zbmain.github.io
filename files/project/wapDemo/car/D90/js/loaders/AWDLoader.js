...
( function () {
	...
	THREE.AWDLoader.prototype = {
		...
		load: function ( url, onLoad,name_str,code_id, onProgress, onError ) {
			...
			loader.load( url, function ( text ,_dkname,_dkcode) {

				//onLoad( scope.parse( text ) ,"_code");
				onLoad( scope.parse( text ) ,name_str,code_id);
		...
		parse: function ( data ) {
		  ...
		  //  console.log( "---加载物体名称:AWDLoader",data.byteLength);
			var blen = data.byteLength;

			this._ptr = 0;
			this._data = new DataView( data );
			
			//console.log( "---加载物体名称:AWDLoader",this._data);

			this._parseHeader( );

		//	alert(this._compression)
